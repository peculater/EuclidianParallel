	...
	class NonLeafNodeTrait extends NodeTrait implements Leafness<DATA> {
		...
		public void doAddData(DATA data, double distance) {
			...
			try {
				...
				IndexItem dollar_underscore = thisNode.children.remove(child.data);
				assert dollar_underscore != null;
			...
			while(!newChildren.isEmpty()) {
				...
				if(thisNode.children.containsKey(newChild.data)) {
					...
					try {
						...
						IndexItem dollar_underscore = thisNode.children.remove(existingChild.data);
						assert dollar_underscore != null;
		...
		private Node balanceChildren(Node theChild) {
			...
			if(nearestDonor == null) {
				...
				IndexItem dollar_underscore = nearestDonor.children.remove(nearestGrandchild.data);
				assert dollar_underscore != null;
